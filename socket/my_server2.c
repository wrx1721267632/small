...
void do_it(int conn_fd)
{
    ...
    while(1){
        if(recv(conn_fd, packet_data, MAX_TO, 0) == 0){         			
    	    	users[i].fd = -1;
    	    	return ;
    	}
    	switch(packet_data.type){
    		case 3:
    			recv(conn_fd, packet_t,sizeof(struct shangxian), 0);
    	    	if((i = find_user(packet_t.username,packet_t.password)) > 0){
    	   			if(users[i].fd == -1){
   	 	    			users[i].fd = conn_fd;
   		    			send(conn_fd, "y\n", 3, 0);
   		    			strcpy(packet_data.data,"服务器： welcome to it !");
		    			if(send (conn_fd, packet_data, sizeof(struct stat), 0) < 0) {
  	 	    			...
  	 	    			time (&timep);
   	 	    			...
   	 	    			strcpy(packet_data.data, recv_buf);
   	 	    			for(j=0; users[j].username[0] != ' '; j++) {
   	 	    				if((users[j].fd != -1) && (i != j)){
   	 	    	   			send(users[j].fd, packet_data.data, sizeof(struct stat), 0);
   	 	    	    		strcpy(recv_buf, users[j].username);
  	 	    	    		recv_buf[strlen(recv_buf)-1] = '\0';
   	 	    	    		strcat(recv_buf, " is online");
   	 	    	    		strcpy(packet_data.data, recv_buf);
   	 	    	    		send(users[i].fd, packet_data.data, sizeof(struct stat), 0);
   	 	    				}
   	 	    			}
 	   	    			break;
 	   				}
  	  					...
  	  					printf("user :%d\n", users[i].fd);
  	  					strcpy(packet_data.data,"n\n");
		    			send(conn_fd, packet_data.data, sizeof(struct stat), 0);
		    			time (&timep);
		    			printf("accept error!\n");
		    			printf("\033[01;34m       %s\033[0m\n",ctime(&timep));
   	 					}
    	    ...
    	    if(users[j].username[0] == ' '){
   		    ...
   		    while(1){
    			...
    			printf("%s \n", recv_buf);
   		    	...
   		    	//printf("i = %d, j = %d\n",users[i].fd,users[j].fd);
   		    	//puts(recv_buf);
   		    	if(send(beiyong_fd, send_buf, MAX_TO, 0) < 0){
   		    		printf("error!\n");
