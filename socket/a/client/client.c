...
//将聊天记录文件发送给服务器
void get_username(char *input_buf);
//获取用户名的函数
...
//======================================================发送文件的函数=================================
//将标志和内容放在一起发void send_file(int conn_fd)送，收到特殊标志的信息后，特殊处理，将第一个空格后的文件内容取出
void send_file(int conn_fd)
//服务器端的套接字
{
	int fd;
	...
	while(i = read(fd, a, 100-j-1)){
		strcat(b, c);
		memcpy(b+j, a,sizeof(a));
		ch = (int) (i + 20);
		b[99] = ch;
...
void output_data(int conn_fd)
{
    ...
    while(1){
        ...
        if(strcmp(recv_buf,"#show_person\n") == 0){
            while(1){
                recv(conn_fd, recv_buf, MAX_TO, 0);
                if(strcmp(recv_buf, "#it,s over\n") == 0){
                    break;
                }
                printf("%s",recv_buf);
            }
        ...
        if((strcmp(recv_buf, "don,t have the qun!") == 0) || (strcmp(recv_buf, "you don,t in the qun!") == 0)){
            type == 0;
        }
        
        if((strcmp(recv_buf, "服务器： don't have the person!\n") == 0) || (strcmp(recv_buf, "服务器： don't online\n") == 0) || (strcmp(recv_buf, "服务器： you can,t talk with yourself\n") == 0)) {
            signal = 2;
            type = 0;
            //当查询到无此用户名时，将不再进行私聊
        } 
...
{
	printf("**********************************************************************************************\n");
	printf(" #to ABC---------------------------给ABC发送私聊消息\n");
	printf(" #all------------------------------给所有人发送群聊消息\n");
	printf(" #file abc.txt---------------------给正在进行私聊的人发送文件abc.txt（发来的文件会在其名字前加@###@）\n");
	printf(" #lixianto ABC---------------------给ABC发送离线消息\n");
	printf(" #exit-----------------------------退出当前模式（退出私聊，群聊等）\n");
	printf(" -exit-----------------------------退出客户端\n");
	printf(" #xiugai---------------------------修改密码\n");
	printf(" #show-----------------------------显示当前用户\n");
	printf(" #jilu-----------------------------显示聊天记录\n");
	printf(" #help-----------------------------显示帮助\n");
    printf(" #create ABC-----------------------创建群ABC\n");
    printf(" #add ABC--------------------------加入群ABC\n");
    printf(" #showqun--------------------------显示群\n");
    printf(" #showperson ABC-------------------显示群ABC的人员\n");
    printf(" #delete ABC-----------------------销毁群(仅创建群的人可以)\n");
    printf(" #goaway ABC-----------------------退出群\n");
	printf("**********************************************************************************************\n");
...
void input_data(int conn_fd)
{
    ...
    while(1) {
        ...
        if(strncmp(input_buf, "#lixianto ", 10) == 0){
        	type = 4;
        }
        
        if((type == 0) && (strcmp(input_buf, "#jilu\n") == 0)){
        	pthread_mutex_lock(&mutex);
			fp = open(username_all, O_RDONLY|O_CREAT, 0777);
			while(1){
				i = read(fp, a, 99);
				//printf("%s",a);					
				memset(a,0,sizeof(a));
        ...
        if(type == 1){
        	...
        	MOVEUP(1);
        	time (&timep);
        	printf("\033[01;35m \rinput(to %s): %s\n      %s\n\033[0m", buf,input_buf,ctime(&timep));
        	send(conn_fd, input_buf, MAX_TO,0);
        ...
        if(type == 4){
        	MOVEUP(1);
        	time (&timep);
        	printf("\033[01;35m \rinput(unline new): %s\n      %s\n\033[0m", input_buf,ctime(&timep));
        	send(conn_fd, input_buf, MAX_TO,0);
        	if(strcmp(input_buf,"-exit\n") == 0){
            	fd = open(username_all, O_RDONLY);
				while(1){
        ...
        if(type == 5){
            MOVEUP(1);
            time (&timep);
        	printf("\033[01;35m \rinput(qun %s): %s\n      %s\n\033[0m", buf, input_buf, ctime(&timep));
            send(conn_fd, input_buf, 100, 0);
        	if(strcmp(input_buf,"-exit\n") == 0){
            	fd = open(username_all, O_RDONLY);
				while(1){
					i = read(fd, a, 99);
